...
export class SearchlegislatorsComponent  implements OnInit {
  ...
  selectedlegislator = {} as Legislator;
  ...
  stateData: boolean = false;
  congressData: boolean = false;
...
/*
  ...
  processOCD(result:any){
    ...
    let divisionsObj = result['divisions'];
    ...
    for (var i = 0; i < divisionsKeys.length; i++) {//"ocd-division/country:us/state:pa/cd:18"
      ...
      divisionData = divisionsObj[divisionsKey];
      ...
      for (var b = 0; b < divisionsKeySplited.length; b++) {//["ocd-division", "country:us", "state:pa", "cd:18"]
        if(divisionsKeySplited[b] !== 'ocd-division'){
          let divisionUnit = divisionsKeySplited[b].split(':'); 
...
//
                  let legislator:Legislator = new Legislator();
                  let legisRoleObj:any;
  ...
  selectDivision(division:string){
    ...
    this.offices.forEach(element1 => {
      let element:any = element1;
      if(element[division]){
