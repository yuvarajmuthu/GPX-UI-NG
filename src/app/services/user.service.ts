...
export class UserService extends AbstractService{
  ...
  getUserData(userId:string, requestorId:string):Observable<any> { 
    ...
    console.log("getUserData()::User service " + url);
  ...
  connectUser(request:string):Observable<any>{
    let serviceUrl = this.getSocialService();
    serviceUrl = serviceUrl + "/followPerson";
    console.log("Connect User user.service " + request + " serviceUrl " + serviceUrl);
     ...
     return this.http.post(serviceUrl, body, {headers, responseType: 'text', observe: 'response'})
      .pipe(
        ...
        map(res => {
          ...
          let authTok = res.headers.get('Authorization');
          if(authTok){
            var tokenBearer = authTok.split(' ');
      ...
      const body = {  };
       ...
       return this.http.post(serviceUrl, body, {headers, responseType: 'text', observe: 'response'})
        .pipe(
          //map((response:Response) => response.json()),
          map(res => {
            console.log("tokenVerify response ", res);
            let authTok = res.headers.get('Authorization');
            if(authTok){
              var tokenBearer = authTok.split(' ');
              if(tokenBearer.length === 2){ 
